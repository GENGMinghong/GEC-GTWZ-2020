...
def get_highlighted_sentence(s1,s2):
    ...
    if indices:
        for indice in indices:
            output[indice-1] = "".join(["<mark>",output[indice-1]]) 
            output[indice+1] = "".join([output[indice+1],"</mark>"])
        output = [x for x in output if x!="<insert_word>"]
    output = ' '.join(output)
    output = output.replace("<replace>","<mark>").replace("</replace>","</mark>")
    output = output.replace("<insert>","<mark>").replace("</insert>","</mark>")
    output = output.replace("<delete>","<mark>").replace("</delete>","</mark>")
    return output